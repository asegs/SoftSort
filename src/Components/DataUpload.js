...
class DataUpload extends Component{
    ...
    let autoSubmit = <p> </p>;
        ...
        // if (this.state.schemaArr.length>0&&this.state.minMax.length>0){
        //     schemaRefinements.push(<GeneralPopover title={"Holy smokes, this looks like a lot!"} body={"Well, it is.  Handwriting details for your inventory is not a quick process.  You don't need to do this all at once, and the program will still work with half finished advanced schema.  Even if you don't give descriptions, numerics will still work a lot better due to units and min/maxes!"}/>)
        //     let schemaTitles = this.state.schemaArr[this.state.schemaArr.length-1];
        //     let newAdvancedSchema = [];
        //     let firstCounter = 0;
        //     if (this.state.advancedSchema.length===0){
        //         for (let step = 0; step < schemaTitles.length; step++) {
        //             if (schemaTitles[step].includes("<M>")) {
        //                 newAdvancedSchema.push([schemaTitles[step],"",this.state.minMax[step][0],this.state.minMax[step][1],""]);
        //             }
        //             else{
        //                 let tempArr = [schemaTitles[step],""];
        //                 for (let i=0;i<this.state.schemaArr[firstCounter].length;i++){
        //                     tempArr.push([this.state.schemaArr[firstCounter][i],""]);
        //                 }
        //                 newAdvancedSchema.push(tempArr);
        //                 firstCounter++;
        //             }
        //         }
        //         this.setState({advancedSchema:newAdvancedSchema});
        //     }
        //     let standardCounter = 0;
        //     for (let step=0;step<schemaTitles.length;step++){
        //         if (schemaTitles[step].includes("<M>")){
        //             schemaRefinements.push(<SchemaClarification edit={this.changeAdvancedAtIndex} num={step} numerical={true} min={this.state.minMax[step][0]} max={this.state.minMax[step][1]} title={schemaTitles[step].substr(0,schemaTitles[step].length-3)}/>);
        //         }
        //         else{
        //             schemaRefinements.push(<SchemaClarification edit={this.changeAdvancedAtIndex} num={step} numerical={false} title={schemaTitles[step]} options={this.state.schemaArr[standardCounter]}/>);
        //             standardCounter++;
        //         }
        //         schemaRefinements.push(<div><br/><br/></div>)
        //     }
        //     schemaRefinements.push(<Button onClick={this.submitNewAdvancedSchema}>Upload advanced schema (doesn't need to be finished!)</Button> );
        // }
        return(
          <div>
